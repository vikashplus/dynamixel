...
@click.option(
    "--protocol", "-p", type=int, help="communication protocol 1/2", default=2
)
def main(motor_id, motor_type, device, baudrate, protocol):

    dxl_goal_position = [
        DXL_MINIMUM_POSITION_VALUE * POS_SCALE,
        DXL_MAXIMUM_POSITION_VALUE * POS_SCALE,
    ]  # Goal position
    index = 0
    ...
    cnt = 10
    print(
        "Testing timings (avg over %d trials for %d motors) -------------"
        % (cnt, len(dxl_ids))
    )
    t_s = time.time()
    for i in range(cnt):
    ...
    t_e = time.time()
    print(
        "Bulk [pos+vel] read takes:\t\t %0.5f sec **BEST**" % ((t_e - t_s) / float(cnt))
    )
    ...
    t_e = time.time()
    print(
        "Bulk [pos+vel,ctrl] loop takes:\t\t %0.5f sec **BEST**"
        % ((t_e - t_s) / float(cnt))
    )
    ...
    for i in range(cnt):
        ...
        for j in range(len(dxl_ids)):
            print(
                "cnt:%03d, dxl_id:%01d ==> Pos:%2.2f, Vel:%1.3f"
    ...
    while 1:
        ...
        while 1:
            ...
            for j in range(len(dxl_ids)):
                print(
                    "cnt:%03d, dxl_id:%01d ==> GoalPos:%2.2f, Pos:%2.2f, Vel:%1.3f"
                    % (
                        cnt,
                        dxl_ids[j],
                        dxl_goal_position[index],
                        dxl_present_position[j],
                        dxl_present_velocity[j],
                    )
                )

            cnt = cnt + 1
            if not (
                abs(dxl_goal_position[index] - dxl_present_position[0])
